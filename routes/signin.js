...
Router.post('/sign-in', (req, res) => {
    ...
    db.all(verifyData, [req.body.firstname, req.body.lastname, req.body.email], (err, row) => {
        ...
        if (Object.keys(row).length === 0) {
            ...
            var sqlintructions = `INSERT INTO User (firstname, lastname, email,password) VALUES (?,?,?,?);`
            db.run(sqlintructions, data, (err, row) => {
                if (err) {
                    ...
                    res.render('../templates/views/log-in', { msg: "Successfully Sign-in" })
            ...
            var SignInMssg = 'This informations was given by a previous user. Please try again'
            res.render('../templates/views/sign-in', { title: " Sign-in", msg: SignInMssg });
        }
